...
public class ChatClient extends JFrame {
    ...
    private JButton send, cancel, sendFile,join,create,leave,lusers,lrooms,saudio,eaudio,caudio;

    private boolean recordingAudio = false,send_record=false;
	...
	//ImageIcon icon = createImageIcon("chat4.png");
	//ImageIcon tab1Icon = new ImageIcon(this.getClass().getResource("chat4.png"));
        roomsPane = new JTabbedPane(SwingConstants.TOP);
	//roomsPane = new JTabbedPane(new ImageIcon("chat5.png"));
        roomsPane.addTab("Chat room",globalRoom);
	
	//roomsPane.setOpaque(false);
	//roomsPane.setBackground(Color.green);
      	//roomsPane.setForeground(Color.black);
        add(roomsPane);

        messageToSend = new JTextField(30);
	...
	send.setForeground(color2);
       	send.setBackground(color1);
        send.addActionListener(ae -> sendStringMessage());
        add(send);

        cancel = new JButton("Cancel");
	cancel.setForeground(color2);
       	cancel.setBackground(color1);
        cancel.addActionListener(ae -> messageToSend.setText(""));
        add(cancel);

        sendFile = new JButton("Send File");
	sendFile.setForeground(color2);
       	sendFile.setBackground(color1);
        sendFile.addActionListener(ae -> sendFile());
        add(sendFile);
	
	join = new JButton("Join Room");
	join.setForeground(color2);
       	join.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	join.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			String rname = JOptionPane.showInputDialog("Enter Room ID");  
			messageToSend.setText("/joinroom "+rname);
			sendStringMessage();
                    }  
                });          
	add(join);
	create = new JButton("Create Room");
	create.setForeground(color2);
       	create.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	create.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			String rname = JOptionPane.showInputDialog("Enter Room Name");  
			messageToSend.setText("/createroom "+rname);
			sendStringMessage();
                    }  
                });          
	add(create);
	leave = new JButton("Leave Room");
	leave.setForeground(color2);
       	leave.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	leave.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			//String rname = JOptionPane.showInputDialog("Enter Room ID");  
			messageToSend.setText("/leaveroom");
			sendStringMessage();
                    }  
                });          
	add(leave);
	lusers = new JButton("List Users");
	lusers.setForeground(color2);
       	lusers.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	lusers.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			//String rname = JOptionPane.showInputDialog("Enter Room ID");  
			messageToSend.setText("/listusers");
			sendStringMessage();
                    }  
                });          
	add(lusers);
	lrooms = new JButton("List Rooms");
	lrooms.setForeground(color2);
       	lrooms.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	lrooms.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			//String rname = JOptionPane.showInputDialog("Enter Room ID");  
			messageToSend.setText("/listrooms");
			sendStringMessage();
                    }  
                });          
	add(lrooms);
	saudio = new JButton("Start Recording");
	saudio.setForeground(color2);
       	saudio.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	saudio.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			recordingAudio = true;
                    sendAudio();
                    }  
	...
	eaudio = new JButton("Stop & Send");
	eaudio.setForeground(color2);
       	eaudio.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	eaudio.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			send_record=true;
	...
	add(eaudio);
	caudio = new JButton("Cancel Recording");
	caudio.setForeground(color2);
       	caudio.setBackground(color1);
	
        //join.addActionListener(messageToSend.setText("/joinroom yoyo");ae -> sendStringMessage());
	caudio.addActionListener(new ActionListener(){  
            public void actionPerformed(ActionEvent e){
			send_record=false;
			recordingAudio = false;
                   // sendAudio();
	...
	add(caudio);
	//add(new JLabel(new ImageIcon("chat1.png")));
